...
def run():
    ...
    #    json.dump(data, f)
    ## retrieve and clean first
    publication = Publication.objects.all()
    for x in publication: x.delete()
    ## Update
    for elem in data[::-1]:
        # Create the Product instance
        pub = Publication(
            title=elem.get('title', 'No Title'),
            year=elem.get('year', 2005),
            authors=elem.get('authors', 'No Authors'),
            link=elem.get('link', 'No Link'),
            journal=elem.get('publication', 'No Journal'),
